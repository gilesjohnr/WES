...
<center>![](../figures/plot_ct_n_copies.png){width="95%"}</center>
...
<center>![](../figures/plot_n_copies.png){width="65%"}</center>

The code below shows how to use the `calc_n_copies()` function on the `template_WES_data` and `template_WES_standard_curve` objects to calculate the number of target gene copies.
...
<center>![](../figures/plot_predicted_n_copies.png){width="70%"}</center>
...
Since environmental sampling data typically do not have a control group, we use the temporal method in the `WES` package to relate all "treatment" observations at time $t$ to the "control" observations at time $t=0$; see Section 1.4 of @livak2001. This means we can write Equation 1 above as: 
...
The following code blocks show how to calculate both the standard and adjusted $\Delta \Delta \text{Ct}$ values for a simulated environmental sampling data set shown in the `template_WES_data` and `template_WES_standard_curve` data objects. To calculate the standard $\Delta \Delta \text{Ct}$ value (Equation 1) initially described in @livak2001 we assume a PCR efficiency of 1 for all targets. The following code does the calculation for a single observation.
...
<center>![](../figures/plot_ddct.png){width="100%"}</center>
