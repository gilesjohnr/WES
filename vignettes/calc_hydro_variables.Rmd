...
See below for an example of how to use the `WES` package to calculate these hydrological variables for a set of sampling sites.
...
The Digital Elevation Model (DEM) is downloaded to the temp directory.
...
The intermediate spatial data produced in the calculation of topographical and hydrological variables are also saved to the temp output directory.
...
rast_slp <- raster::raster(file.path(tempdir(), 'slope.tif'))
rast_asp <- raster::raster(file.path(tempdir(), 'aspect.tif'))
rast_twi <- raster::raster(file.path(tempdir(), 'twi.tif'))
rast_acc <- raster::raster(file.path(tempdir(), 'flow_acc.tif'))
rast_hill <- raster::raster(file.path(tempdir(), 'hillshade.tif'))
shp_streams <- sf::st_read(file.path(tempdir(), 'streams.shp'), quiet=TRUE)
